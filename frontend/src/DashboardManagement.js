...
function formatShortText(value, maxLength = 140) {
...
function getWeeklyReasonLabel(item) {
  return formatShortText(item.description || item.complaint_type || 'Nao informado', 180);
}
...
function DashboardManagement() {
  ...
  const canViewWeeklyReport = isWeeklyComplaintReportAllowed(currentUser);
  ...
  const weeklySummaryRows = useMemo(() => weeklyComplaints.map((item) => ({
  ...
  const weeklyPreviewRows = useMemo(() => weeklySummaryRows.slice(0, 10), [weeklySummaryRows]);
  ...
  const exportWeeklyExcel = () => {
    const headers = Object.keys(weeklySummaryRows[0] || {
      protocolo: '',
      data_cadastro: '',
      paciente: '',
      clinica: '',
    ...
    const html = `
      <table>
        <thead><tr>${headers.map((header) => `<th>${escapeHtml(header)}</th>`).join('')}</tr></thead>
        <tbody>
          ${weeklySummaryRows.map((row) => `<tr>${headers.map((header) => `<td>${escapeHtml(row[header])}</td>`).join('')}</tr>`).join('')}
        </tbody>
      </table>
    `;
    const blob = new Blob([html], { type: 'application/vnd.ms-excel;charset=utf-8;' });
  ...
  const exportWeeklyPdf = () => {
    const printDate = new Date();
    const rows = weeklySummaryRows.map((row) => [
      row.protocolo,
      row.data_cadastro,
      row.paciente,
      row.clinica,
          ...
          <main class="report-shell">
            ...
            <section class="summary-grid">
              <article class="summary-card">
                ...
                <span>${escapeHtml(String(weeklyHighlights.total))}</span>
              </article>
              <article class="summary-card">
                <strong>Clinicas envolvidas</strong>
                <span>${escapeHtml(String(weeklyHighlights.clinics))}</span>
              </article>
              <article class="summary-card">
                <strong>Frentes envolvidas</strong>
                <span>${escapeHtml(String(weeklyHighlights.professionals))}</span>
              </article>
              <article class="summary-card">
        ...
        <section className="management-panel weekly-report-panel">
          <div className="panel-heading">
            <div>
              ...
              <h2>Reclamacoes registradas nos ultimos 7 dias</h2>
              <p className="panel-supporting-copy">
                Visao executiva com nome da paciente, clinica, profissional envolvido e motivo principal da reclamacao.
              </p>
            </div>

            <div className="export-actions">
              <button className="outline-action" onClick={exportWeeklyExcel} disabled={!weeklySummaryRows.length}>
                <span className="export-badge excel">XLS</span>
                <span>Baixar Excel</span>
              </button>
              <button className="outline-action" onClick={exportWeeklyPdf} disabled={!weeklySummaryRows.length}>
          ...
          <div className="weekly-report-metrics">
            <article className="weekly-report-card">
              ...
              <strong>{weeklyHighlights.total}</strong>
              <p>Registros criados na janela atual</p>
            </article>
            <article className="weekly-report-card">
              <span>Clinicas envolvidas</span>
              <strong>{weeklyHighlights.clinics}</strong>
              <p>Unidades com demanda registrada</p>
            </article>
            <article className="weekly-report-card">
              <span>Frentes envolvidas</span>
              <strong>{weeklyHighlights.professionals}</strong>
              <p>Servicos ou responsaveis citados</p>
            </article>
            <article className="weekly-report-card">
              <span>Disponivel para</span>
              <strong>Perfis de gestao</strong>
              <p>Master, administrador, supervisor CRC, SAC e gerente</p>
            </article>
          </div>
